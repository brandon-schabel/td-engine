...
export const NotificationToast: React.FC<NotificationToastProps> = ({
  ...
  const { id, message, type = 'info', duration = 3000, icon, action } = notification;
...
export function useToastNotifications() {
  ...
  const success = useCallback((message: string, options?: Partial<ToastNotification>) => {
    return show({ ...options, message, type: 'success' });
  }, [show]);

  const error = useCallback((message: string, options?: Partial<ToastNotification>) => {
    return show({ ...options, message, type: 'error' });
  }, [show]);

  const info = useCallback((message: string, options?: Partial<ToastNotification>) => {
    return show({ ...options, message, type: 'info' });
  }, [show]);

  const warning = useCallback((message: string, options?: Partial<ToastNotification>) => {
    return show({ ...options, message, type: 'warning' });
  }, [show]);

  const ToastContainer = useCallback(({ position }: { position?: 'top' | 'bottom' }) => (

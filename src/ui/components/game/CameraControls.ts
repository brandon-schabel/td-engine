...
import type { ComponentProps, ComponentState } from "@/ui/core/types";
import { StyleSystem } from "@/ui/core/StyleSystem";
  ...
  protected render(): HTMLElement {
    ...
    if (compact && !expanded) {
      container.innerHTML = `
        <button class="${
          styles.expandButton
        }" data-action="expand" title="Camera Controls">
    ...
    container.innerHTML = `
        ${
          ...
          }" data-action="collapse" title="Collapse">
        ...
        <div class="${styles.header}">
          ...
          ${
            showZoomLevel
              ? `<span class="${styles.zoomLevel}">${zoomPercentage}%</span>`
              : ""
          }
        </div>
        ...
        <div class="${styles.controls}">
          <div class="${styles.zoomControls}">
            ...
            <button class="${
              styles.button
            }" data-action="zoom-reset" title="Reset Zoom (0)">
            ...
            <button class="${
              styles.button
            }" data-action="zoom-fit" title="Fit to Screen (F)">
    ...
    }" data-action="toggle-follow" title="Toggle Follow Player (C)">
              ${createSvgIcon(followIcon, { size: 18 })}
              ${
                showLabels
                  ? `<span>${isFollowing ? "Following" : "Free"}</span>`
                  : ""
              }
            </button>
          </div>

...
export class TowerUpgradePanel extends GameComponent<
  TowerUpgradePanelProps,
  TowerUpgradePanelState
> {
  ...
  private upgradeTower = (upgradeType: UpgradeType) => {
    if (this.state.selectedTower) {
      const success = this.game.upgradeTower(
      ...
      if (success) {
        ...
        this.uiManager.showNotification(
          `Tower ${upgradeType.toLowerCase()} upgraded!`,
          "success"
        );
      } else {
        // Show error feedback
  ...
  private createHeader(): HTMLElement {
    const Header = styled.div`
      ...
      border-bottom: 1px solid
        ${(props: { theme: any }) => props.theme.colors.border};
    `;
  ...
  private createTowerStats(): HTMLElement {
    ...
    const stats = [
      ...
      {
        label: "Fire Rate",
        value: `${tower.fireRate.toFixed(1)}/s`,
        icon: "⚡",
      },
  ...
  private createUpgradeButton(upgrade: UpgradeOption): Button {
    ...
    const canUpgrade =
      tower.canUpgrade(upgrade.type) &&
      this.game.canAffordUpgrade(tower, upgrade.type);
    ...
    if (buttonEl) {
      ...
      if (cost > 0) {
        rightSide.innerHTML = `<div style="color: #FFD700; font-weight: bold;">${this.formatCurrency(
          cost
        )}</div><div style="color: #4CAF50;">+${increase}</div>`;
      } else {
  ...
  private getCurrentUpgradeValue(
    tower: Tower,
  ...
  protected override onStateUpdate(prevState: TowerUpgradePanelState): void {
    ...
    if (
      prevState.selectedTower !== this.state.selectedTower ||

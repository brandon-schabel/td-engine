...
export class EnemyInfoPopup extends EntityInfoPopup {
  ...
  private updateSections(): void {
    ...
    sections.push({
      ...
      icon: IconType.SPEED,
      format: (v: string | number) => `${Number(v)}/s`
    });
    
    // Armor (if implemented in the future)
    // if (this.enemy.armor && this.enemy.armor > 0) {
    //   sections.push({
    //     label: 'Armor',
    ...
    if (this.enemyOptions.showReward) {
      ...
      sections.push({
        ...
        format: (v: string | number) => formatNumber(Number(v))
      });
    }
    
    // Status effects (if implemented in the future)
  ...
  private static getEnemyColor(enemy: Enemy): string {
    switch (enemy.enemyType) {
      case EnemyType.BASIC:
        return COLOR_THEME.enemies.basic;
      case EnemyType.FAST:
        return COLOR_THEME.enemies.fast;
      case EnemyType.TANK:
        return COLOR_THEME.enemies.tank;
      default:

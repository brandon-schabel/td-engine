...
const isInViewport = (entity: { position: Vector2 }, viewport: Rectangle): boolean => {
  // Add null checks for entity and position
...
export const utilizeEntityStore = create<EntityStore>()(
  devtools(
    immer((set, get) => ({
      // Initial state
      towers: {},
      ...
      addEnemy: (enemy) => set((state) => {
        // Validate enemy has a valid position before adding
        if (!enemy || !enemy.position || 
            typeof enemy.position.x !== 'number' || 
            typeof enemy.position.y !== 'number') {
          console.warn('EntityStore: Attempted to add enemy with invalid position', enemy);
          return;
        }
...
export const useTowers = () => utilizeEntityStore(state => state.getAllTowers());
export const useEnemies = () => utilizeEntityStore(state => state.getAllEnemies());
export const useProjectiles = () => utilizeEntityStore(state => state.getAllProjectiles());
export const useCollectibles = () => utilizeEntityStore(state => state.getAllCollectibles());
export const useDestructionEffects = () => utilizeEntityStore(state => state.getAllDestructionEffects());
export const usePlayer = () => utilizeEntityStore(state => state.player);
export const useSelectedTower = () => utilizeEntityStore(state => state.selectedTower);
export const useHoveredTower = () => utilizeEntityStore(state => state.hoveredTower);
export const useEntityCount = () => utilizeEntityStore(state => state.getEntityCount());

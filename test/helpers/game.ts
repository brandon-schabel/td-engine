...
import { TowerUpgradeManager } from '@/systems/TowerUpgradeManager';
...
export function simulateGameFrames(game: Game, frameCount: number, deltaTime = 16): void {
...
export function getTowerAt(game: Game, gridX: number, gridY: number): Tower | undefined {
  const gameAny = game as any;
...
export function getEnemiesInRange(game: Game, position: Vector2, range: number): Enemy[] {
  const gameAny = game as any;

...
describeSystem('Grid',
  ...
  (getGrid, context) => {
    ...
    describe('cell operations', () => {
      ...
      it(then('returns BLOCKED for out of bounds'), () => {
        const grid = getGrid();
    ...
    describe('coordinate conversion', () => {
      it(when('converting world to grid'), () => {
        const grid = getGrid();
      ...
      it(when('converting grid to world'), () => {
        const grid = getGrid();
    ...
    describe('placement validation', () => {
      ...
      it(then('cannot place on path'), () => {
        const grid = getGrid();
        grid.setCellType(5, 5, CellType.PATH);
        expect(grid.canPlaceTower(5, 5)).toBe(false);
      });

      it(then('cannot place on blocked'), () => {
        const grid = getGrid();
        grid.setCellType(5, 5, CellType.BLOCKED);
        expect(grid.canPlaceTower(5, 5)).toBe(false);
      });

      it(then('cannot place on existing tower'), () => {
        const grid = getGrid();
        grid.setCellType(5, 5, CellType.TOWER);
        expect(grid.canPlaceTower(5, 5)).toBe(false);
      });

      it(then('cannot place out of bounds'), () => {
        const grid = getGrid();
    ...
    describe('neighbors', () => {
      ...
      it(then('excludes out of bounds'), () => {
        const grid = getGrid();
      ...
      it(when('getting walkable neighbors'), () => {
        const grid = getGrid();
